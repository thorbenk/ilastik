...
class OpRegionFeatures3d(Operator):
    ...
    def _extract(self, image, labels):
        ...
        tmpfeats = vigra.analysis.extractRegionFeatures(image, labels,
                                                        ["Coord<Minimum>", "Coord<Maximum>"],
                                                        ignoreLabel=0)
        mins = tmpfeats["Coord<Minimum >"]
        maxs = tmpfeats["Coord<Maximum >"]
        nobj = mins.shape[0]
        ...
        feature_dict = self.Features([]).wait()

        # do global features
        for plugin_name, feature_list in feature_dict.iteritems():
            ...
            result = dictextend(result, feats)
        ...
        # FIXME: some features return different shapes. account for all possibilities.
        # FIXME: feature dict should be seperated by channels
        for key, value in result.iteritems():
            if len(value) == nobj - 1:
                value.insert(0, numpy.array([None] * len(value[0])))
            result[key] = numpy.vstack(value)
            if result[key].shape[0] != nobj:
                raise Exception()
